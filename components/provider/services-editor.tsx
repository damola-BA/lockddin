...
function ServiceForm({
  ...
  return (
    <form
      action={formAction}
      className="space-y-4 rounded-lg border border-stone-700 bg-stone-900 p-4"
    >
      ...
      <p className="text-sm text-stone-500">{t.onboarding.priceHint}</p>
      <div>
        ...
        <TextInput
          ...
          placeholder="—"
        />
        <p className="mt-1.5 text-sm text-stone-500">{t.onboarding.bufferOverrideHint}</p>
      </div>
      <div>
        ...
        <textarea
          ...
          rows={3}
          className="w-full rounded-lg border border-stone-700 bg-stone-900 px-3.5 py-3 text-base text-stone-100 placeholder:text-stone-600 focus:border-amber-400 focus:outline-none"
        />
        <p className="mt-1.5 text-sm text-stone-500">{t.onboarding.prepHint}</p>
      </div>
      ...
      <div className="flex gap-3">
        <button
          type="button"
          onClick={onDone}
          className="w-full rounded-lg border border-stone-700 px-4 py-3 text-base text-stone-300"
        >
          {t.common.cancel}
        </button>
        <button
  ...
  return (
    <div className="space-y-3">
      {services.length === 0 && editing !== "new" && (
        <p className="text-sm text-stone-400">{t.onboarding.noServicesYet}</p>
      )}
      ...
      {services.map((service) =>
        editing === service.id ? (
          ...
          <div
            key={service.id}
            className="flex items-start justify-between gap-3 rounded-lg border border-stone-800 bg-stone-900/60 p-4"
          >
            <div>
              <p className="font-medium text-stone-100">{service.name}</p>
              ...
              {service.prep_instructions && (
                <p className="mt-1 text-sm text-stone-500">{service.prep_instructions}</p>
              )}
            </div>
      ...
      {editing === "new" ? (
        ...
        <button
          ...
          className="w-full rounded-lg border border-dashed border-stone-600 px-4 py-3 text-base text-stone-300"
        >
